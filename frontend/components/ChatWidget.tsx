...
export function ChatWidget() {
  ...
  const [msgs, setMsgs]     = useState<Msg[]>([{ role:'assistant', content:'Привіт! Я AI-асистент каталогу. Запитайте про будь-який товар 🔧' }])
  ...
  const send = async () => {
    ...
    try {
      const { reply } = await catalogApi.chat(next)
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)} style={{
      ...
      <span style={{ position:'absolute', inset:0, borderRadius:'50%', border:'2px solid rgba(69,97,248,.5)', animation:'pulse-ring 2s ease-out infinite' }} />
    </button>
  )
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:4 }}>
          <button onClick={e => { e.stopPropagation(); setMini(v=>!v) }} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,.75)', padding:4 }}>
            <Minus size={14} />
          </button>
          <button onClick={() => setOpen(false)} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,.75)', padding:4 }}>
            <X size={14} />
          </button>
        </div>
      </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:14, display:'flex', flexDirection:'column', gap:10 }}>
          {msgs.map((m, i) => (
            <div key={i} style={{ display:'flex', gap:8, flexDirection: m.role==='user' ? 'row-reverse' : 'row' }}>
              <div style={{ width:28, height:28, borderRadius:'50%', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background: m.role==='assistant' ? 'var(--brand)' : 'var(--bg-hover)' }}>
                {m.role==='assistant' ? <Bot size={14} color="#fff" /> : <User size={14} style={{color:'var(--text-2)'}} />}
              </div>
              <div style={{
                maxWidth:'75%', padding:'9px 12px', fontSize:13, lineHeight:1.5,
        ...
        <div style={{ padding:'10px 12px', borderTop:'1px solid var(--border)', flexShrink:0 }}>
          <div style={{ display:'flex', gap:8 }}>
            <textarea
              ...
              placeholder="Запитайте про товар…" rows={1}
          ...
          <p style={{ fontSize:11, color:'var(--text-3)', textAlign:'center', marginTop:6 }}>Enter — надіслати · Shift+Enter — рядок</p>
        </div>
      </>}
    </div>
  )
