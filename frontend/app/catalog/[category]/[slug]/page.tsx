...
export default function SectionPage() {
  ...
  useEffect(() => {
    if (!slug) return
    setLoading(true)
    api.getProductsBySection(slug, page, PAGE_SIZE)
      .then(r => {
        setProducts(r.items)
        setTotal(r.total)
  ...
  useEffect(() => {
    if (!category) return
    api.getCategory(category).then(c => setCatName(c.name)).catch(() => {})
  }, [category])
    ...
    <>
      ...
      <div className="container" style={{ paddingTop: 24 }}>
        <div className="breadcrumbs">
          ...
          <Link href={`/catalog/${category}`}>{catName || category}</Link>
          <span className="breadcrumbs-sep">›</span>
          <span>{sectionName || slug}</span>
        </div>

        <div className="section-header" style={{ marginTop: 8, marginBottom: 24 }}>
          <h1 className="section-title">{sectionName || slug}</h1>
        ...
        {loading ? (
          ...
          <div className="loader-wrap">
            <p style={{ color: 'var(--text2)' }}>Товарів не знайдено</p>
          </div>
        ) : (
          <>
